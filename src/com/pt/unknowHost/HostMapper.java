 ...
 */
public class HostMapper extends Mapper<LongWritable, Text, Text, Text> {

    private final static Log log = LogFactory.getLog(HostMapper.class);
        ...
        String devicetype = userAgent.getDeviceType();
            ...
            //如果是浏览器行为 输出 useragent 标识符\tuseragent\t浏览器 valuebean
                ...
                if (Strings.isNullOrEmpty(HOST)){
                    String[] hosts = (bean.Host).split("[.]",-1);
                    if (hosts.length<2){
                        return;
                    }
                    if (hosts.length >3 && bean.Host.endsWith("cn")){
                        context.write(new Text(hosts[hosts.length - 3] + "." +hosts[hosts.length - 2] + "." + hosts[hosts.length - 1]),new Text(bean.Host));

                    }else {
                        context.write(new Text(hosts[hosts.length - 2] + "." + hosts[hosts.length - 1]), new Text(bean.Host));
                    }
                    }
